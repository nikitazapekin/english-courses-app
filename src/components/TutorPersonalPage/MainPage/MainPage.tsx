...
const MainPage = () => {
    ...
    const [rateInfo, setRateInfo] = useState<Medium>({
        average: 0,
        count: 0
    });
    ...
    const getRate = async () => {
        try {
            const resp = await RatesService.GetMediumRates(tutor.user.id);
            setRateInfo(resp.data.rates);
        } catch (e) {
            console.log(e);
        }
    };
    ...
    const renderStars = () => {
        const stars = [];
        // Округляем по правилам: если десятичная часть не 0 и не 0.5 - округляем вверх
        let roundedRating = Math.round(rateInfo.average * 2) / 2;
        if (rateInfo.average % 1 !== 0 && rateInfo.average % 1 !== 0.5) {
            roundedRating = Math.ceil(rateInfo.average * 2) / 2;
        }
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(
                <div key={`full-${i}`} className={styles.star__star} />
            );
        }
        ...
        if (hasHalfStar) {
            stars.push(
                <div key="half" className={styles.star__starSliced} />
            );
        }
        ...
        for (let i = 0; i < emptyStars; i++) {
            stars.push(
                <div key={`empty-${i}`} className={styles.star__star} style={{ opacity: 0.3 }} />
            );
        }
    ...
    return (
        <div className={styles.main}>
            ...
            <div className={styles.main__content}>
                ...
                <div className={styles.main__field}>
                    ...
                    <div className={styles.star__stars}>
                        {renderStars()}
                        <span className={styles.ratingCount}>({rateInfo.count})</span>
                    </div>
                </div>
            ...
            <button className={styles.btn} onClick={hanldeLogout}>
                Выйти из аккаунта
            </button>
        </div>
    );
