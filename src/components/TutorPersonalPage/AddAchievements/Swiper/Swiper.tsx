...
import { selectAchievement } from '../../../../store/slices/AddAchievementSlice/AddAchievementSlice';
...
function Swiper({ items }: Props) {
    ...
    const onTouchStart = (e: React.TouchEvent | React.MouseEvent) => {
        ...
        setStartX(clientX);
        setIsDragging(false); // Начинаем как не dragging
    ...
    const onTouchMove = (e: React.TouchEvent | React.MouseEvent) => {
        ...
        if (Math.abs(diff) > 5) { // Небольшой порог для начала свайпа
            setIsDragging(true);
        }
                            ...
                            {group.map((video) => (
                                <div key={video.id} className={styles.videoContainer}>
                                    <div 
                                        ...
                                        onClick={() => handleOpen(Number(video.id))}
                                    >
                                        <img 
                                            src={video.logo}
